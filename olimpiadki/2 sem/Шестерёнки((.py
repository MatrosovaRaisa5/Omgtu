//некоторые тесты выполняются, а некоторые нет
def Program(mode, input_path=None, output_path=None):
    gears = []
    connections = []
    if mode == 'file':
        f = open(input_path, "r")
    ...
    if mode == 'auto':
        ...
        f.close()
    return out_str
print(Program(mode="file", input_path="input_s1_03.txt", output_path="output.txt"))
